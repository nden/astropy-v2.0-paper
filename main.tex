...
``normalized convolution'' \citep[e.g.,][]{Knutsson1993} that is an image
...
An example of this is given in \figurename~\ref{fig:convolution-example}.
In \astropypkg versions $\leq 1.3$, the direct convolution and Fast Fourier Transform (FFT)
convolution approaches were inconsistent, with the latter implementing
normalized convolution, while the former a different approach.
...
we show an RGB color image of the Hickson 88 group, centered near NGC~6977.
...
The \package{cosmology} package contains classes for representing different
cosmologies and functions for calculating commonly used quantities such as
look-back time and distance.   The package was described in detail in
